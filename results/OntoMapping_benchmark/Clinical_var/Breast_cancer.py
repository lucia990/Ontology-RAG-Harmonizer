...
def save_intermediate_results_cui(
    result_tuple,
    candidate_file="results/OntoMapping_benchmark/Clinical_var/Breast_cancer/optimized/all_candidates-3.csv",
    review_file="results/OntoMapping_benchmark/Clinical_var/Breast_cancer/optimized/mapping_reviews_cui-3.csv"
):
    candidate_df, review_df = result_tuple
...
if __name__ == "__main__":
    breast_cancer = pd.read_csv(
        'Evaluation/OntoMapping_benchmark/DATA/Breast_cancer.csv'
    )[['optimized_name', 'schemanode_description', 'ontology_id']]

    # Output files
    OUTPUT_DIR = "results/OntoMapping_benchmark/Clinical_var/Breast_cancer/optimized"
    ...
    for _, row in tqdm(breast_cancer.iterrows(), total=len(breast_cancer)):
        var      = row['optimized_name']
